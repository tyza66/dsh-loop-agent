...
window.__ModuleLoader__.load({ id: '@tyza66/dsh-loop-agent', factory: (require) => {
   ...
   * and `host` is the package-private Client→Host channel. State is read
   * once on mount and after every write; there is no live subscription,
   * because the underlying value is a file the Host owns and the only
   * writer is this switch.
   */
  function LoopSection(props) {
    ...
    React.useEffect(function () {
      var cancelled = false
      host.call('loop-agent.getState', {}).then(function (result) {
        if (cancelled) return
        setSnapshot(result)
      })['catch'](function () {
    ...
    function toggle(next) {
      ...
      setErrorMessage('')
      setShowSaved(false)
      host.call('loop-agent.setEnabled', { enabled: next }).then(function (result) {
        setSnapshot(result)
        setShowSaved(true)
      })['catch'](function () {
  ...
  /** Services the browser half needs before it can register anything. */
  var inject = ['slots', 'locale', 'host']
