...
window.__ModuleLoader__.load({ id: '@tyza66/dsh-loop-agent', factory: (require) => {
  ...
  var en = {
    ...
    foreverHint: 'Sessions do not auto-loop. Type /forever in any session to activate its endless loop (only works when the global switch is on). Turning off the switch immediately stops all running loops.',
    enableHint: 'Enable the /forever command mechanism. Sessions must be activated individually with /forever.',
    intro: 'Activate endless loops on-demand: type /forever in any session to make it run forever after each answer.'
  }
  ...
  var zh = {
    ...
    versionLabel: '版本',
    foreverHint: '会话不会自动 loop。在任意会话中输入 /forever 以激活该会话的无尽模式（仅在全局开关打开时生效）。关闭开关会立即停止所有正在运行的 loop。',
    enableHint: '启用 /forever 命令机制。每个会话需要单独输入 /forever 才能激活。',
    intro: '按需激活无尽模式：在任意会话中输入 /forever，让该会话在每次回答后自动继续。'
  }
   ...
   * The section reads the host snapshot on mount and then re-polls it
   * every 2 seconds while the panel stays open, so the "running now" agent
   * count stays live as sessions are archived, stopped, or created. Only
   * the live fields (enabled, count) refresh from a poll; the continuation
   * draft is seeded from the first successful read and then left alone, so
   * a background round-trip never clobbers an in-progress edit. The host
   * is reached over the webserver routes the loop's host half registers —
   * a plain `fetch` is enough; the dynamic-only `host.call` channel does
   * not exist in static dual-face plugins.
   */
  function LoopSection(props) {
    ...
    React.useEffect(function () {
      var cancelled = false
      var seeded = false
      /** GET the host snapshot. The first successful read seeds the
       * continuation draft; later polls refresh only the live fields
       * (`enabled`, the attached count) so a mid-edit textarea is never
       * clobbered by a background round-trip. A successful read also clears
       * a stale "could not reach the host" error after a transient outage. */
      function refresh() {
        return fetch('/api/loop-agent/state', { method: 'GET', headers: { accept: 'application/json' } })
      ...
      /* Read immediately, then keep polling: the agent count changes while
       * the panel is open (sessions archived / stopped by the user / new
       * conversations started), and the snapshot must track that. */
      refresh()
    ...
    return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '760px' } }, [
      ...
      React.createElement('div', {
        ...
        React.createElement('p', { key: 'foreverHint', style: { margin: 0, fontSize: '12px', opacity: 0.6, lineHeight: 1.5 } }, t('foreverHint'))
      ]),

      // Status lines: the live agent count and the saved notice after a
      // write. The count line only makes sense while the loop is on — with
      // the switch off the host reports 0 and the line is hidden, so it can
      // never claim "N running" next to an Off switch. When on, 0 is shown
      // explicitly: after the user stops or archives every conversation the
      // number visibly drops to 0 instead of vanishing ambiguously.
      ...
      // Footer: the bundle version, read from the host snapshot (which
      // reads it from package.json at load — single source of truth, so
      // the badge can never drift from what npm publish shipped). Hidden
      // when the host did not report one (older host half / manifest
      // stripped) rather than showing a placeholder.
