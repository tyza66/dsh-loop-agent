    ...
    const timer = setInterval(() => {
        ...
        for (const [agent, state] of [...states]) {
          ...
          for (const event of events) {
            ...
            if (isForeverCommand(text)) {
              ...
              states.delete(agent);
              // Mark as armed to prevent on-demand re-parking
              armedAgentIds.add(agent.id);
              logger?.info?.(`dsh-loop-agent: agent "${agent.id}" marked for re-arming by /forever command`);
              break;
            }
          }
        }
      ...
      for (const agent of live) {
        ...
        if (armedAgentIds.has(agent.id)) {
          ...
          driveLoop(state, logger).catch(err => {
            logger?.error?.(`dsh-loop-agent: driveLoop failed for agent "${agent.id}":`, err);
          });
          continue;
        }
        const state = new LoopState(agent, config, profile);
