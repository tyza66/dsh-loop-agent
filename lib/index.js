 ...
 * or the host tearing the plugin down. A turn preempted by a new user
 * message (`interrupted`) is neither — the driver waits for that turn to
 * settle and resumes, so an interjected question does not kill the run.
 *
...
var LoopState = class {
   ...
   * (last turn ended `aborted`). Distinct from a plain teardown: the
   * supervisor keeps halted states in its map so it never reattaches a
   * fresh driver, while toggling the loop off and on sweeps them away to
   * re-arm every conversation. */
  haltedByUser = false;
...
function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (enabled) {
         ...
         * the idle-grace guard). Live drivers are untouched. */
   ...
   * (the poll replaces the never-delivered `agent/disposed` signal).
