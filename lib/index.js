...
function apply(ctx, config) {
  ...
  logger?.info?.(`dsh-loop-agent: attempting to register /forever command`);
  ctx.effect(function* () {
    logger?.info?.(`dsh-loop-agent: /forever command registration effect starting`);
    try {
      yield ctx.commands.register({
        name: "forever",
        description: "Activate endless loop for this session",
        handler: async (invocation) => {
          logger?.info?.(`dsh-loop-agent: /forever command handler invoked!`);
          const agent = invocation.agent;
          logger?.info?.(`dsh-loop-agent: /forever command invoked for agent ${agent?.id || "unknown"}`);
          
          if (!agent) {
            logger?.warn?.(`dsh-loop-agent: /forever invoked but no agent in invocation`);
            return {
              kind: "error",
              text: "No active agent in this session."
            };
          }

          // Check if global switch is on
          if (readDisabled(profile)) {
            logger?.warn?.(`dsh-loop-agent: /forever invoked but global switch is off`);
            return {
              kind: "error",
              text: "Endless loop is globally disabled. Turn it on in Settings → Endless loop first."
            };
          }
          ...
          logger?.info?.(`dsh-loop-agent: looking up state for agent ${agent.id}, found: ${!!state}`);
          if (!state) {
            logger?.warn?.(`dsh-loop-agent: agent "${agent.id}" not found in states map`);
            return {
              kind: "error",
              text: "This session is not tracked by the endless loop system."
            };
          }

          // Check if already running
          logger?.info?.(`dsh-loop-agent: state found, disarmed: ${state.disarmed}, haltedByUser: ${state.haltedByUser}`);
          if (!state.disarmed) {
            logger?.info?.(`dsh-loop-agent: loop already active for agent ${agent.id}`);
            return {
              kind: "success",
              text: "Endless loop is already active for this session."
            };
          }

          // Activate the loop by removing the halted state (use the agent from the map)
          logger?.info?.(`dsh-loop-agent: deleting state for agent ${agent.id} to re-arm loop`);
          states.delete(foundAgent);
          logger?.info?.(`dsh-loop-agent: agent "${agent.id}" re-armed by /forever command; supervisor will attach driver on next tick`);

          return {
            kind: "success",
            text: "Endless loop activated. The session will now continue automatically after each answer."
          };
        }
      });
      logger?.info?.(`dsh-loop-agent: /forever command registered successfully`);
    } catch (error) {
      logger?.error?.(`dsh-loop-agent: failed to register /forever command: ${error?.message || error}`);
      throw error;
    }
