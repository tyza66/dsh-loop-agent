...
import { readDisabled, writeDisabled } from "./patch-state.js";
...
const Config = z.object({
   ...
   * (the concatenated assistant text of the previous round), `{{round}}`
   * (1-based; round 1 is the first continuation, not the user's first
   * message), and `{{task}}` (always empty in web mode, kept for parity
   * with the headless variant). Unknown placeholders are left intact.
   */
 ...
 * continuation after the user has torn the session down.
 */
var LoopState = class {
  agent;
  config;
...
async function runOnePhase(state, agent, config, logger, cursor) {
  ...
  // round's assistant text and queue it for the next turn. Round numbering
  // is 1-based and counts continuations only — round 1 is the first
  // continuation, not the user's first message.
  const nextRound = round + 1;
  const nextPrompt = renderTemplate(config.continuation, {
    lastAnswer,
    round: nextRound,
    task: ""
  });
...
function apply(ctx, config) {
   ...
   *
   * `enabled` is the *user layer's* answer, not this row's mount state:
   * the settings page has to show what the next boot will do, which is
   * the composition of this bundle's `disabled: false` with whatever the
   * person's own patch file overrides it with.
   */
  const store = {
    /** True when the user layer does not disable `loop-runner`. */
    get enabled() {
  ...
  /**
   * Client→Host RPC surface for the settings section. `harness` is the
   * package-private channel: the browser half calls `host.call(method,
   * args)` and only lossless JSON crosses the boundary.
   */
  const harness = ctx.get("harness");
  harness?.handle?.("loop-agent.getState", async () => ({
    enabled: store.enabled,
    attachedAgents: store.attachedAgents,
    profile
  }));
  harness?.handle?.("loop-agent.setEnabled", async (args) => {
    const enabled = args?.enabled === true;
    const result = writeDisabled(!enabled, profile);
    logger?.info?.(`dsh-loop-agent: endless loop ${enabled ? "enabled" : "disabled"} in ${result.path}; takes effect on the next profile start`);
    return {
      enabled: store.enabled,
      attachedAgents: store.attachedAgents,
      path: result.path,
  ...
  ctx.effect(function* () {
    ...
    const onCreated = ctx.on("agent/created", ({ agent }) => {
      ...
      const state = new LoopState(agent, config);
