...
async function driveLoop(state, logger, compaction) {
  ...
  let lastRoundEndSeq;
  try {
    lastRoundEndSeq = seedInitialBoundary(agent.session.events);
  } catch (error) {
    ...
    handler: async (invocation) => {
      try {
        ...
        if (!state) {
          ...
          state.haltAtSeq = agent.session?.events?.length ?? 0; // Only scan NEW messages
          state.haltedByUser = true;
          state.disarm();
  ...
  ctx.effect(() => {
    ...
    const timer = setInterval(() => {
      ...
      for (const agent of live) {
        ...
        const state = new LoopState(agent, config, profile);
        state.haltAtSeq = agent.session?.events?.length ?? 0; // Only scan NEW messages, not history
        state.haltedByUser = true;
        state.disarm();
        states.set(agent, state);
