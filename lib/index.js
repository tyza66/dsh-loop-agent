...
function apply(ctx, config) {
  ...
  /**
   * Client→Host bridge: two webserver routes the browser half calls from its
   * settings section. The contract is a stable JSON shape; the host side
   * always answers with the current `loopConfig` snapshot plus a `path` /
   * `changed` pair for writes, so the UI can tell the user exactly which
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: (req, res) => {
      ...
      sendJson(res, 200, readEnabledState());
    }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      for await (const chunk of req) {
        total += chunk.length;
        if (total > MAX) { tooLarge = true; break; }
        chunks.push(chunk);
      }
      if (tooLarge) {
