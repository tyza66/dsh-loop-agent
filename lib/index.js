 ...
 * choice, or the first option) and a tool call that needs one-shot approval
 * is granted `allowed-once`. Both only apply to agents this plugin drives
 * (loop on, driver armed), are configurable (`autoAnswerQuestions` /
 * `autoApproveActions`), never guess free-text answers, and leave sessions
 * the user is interacting with directly on the normal ask/approve UI.
 *
...
const Config = z.object({
   ...
   * option. Questions with no options at all (free text) are never guessed
   * — the whole batch is passed through to the human. Only agents this
   * plugin drives (loop on, driver armed) are affected; interactive
   * sessions keep asking normally.
   */
 ...
 * with) when EVERY question can be answered from its options, or null when
 * any question is free-text — a text answer is a genuine judgment call, so
 * the whole batch falls through to the human rather than partially
 * auto-answering.
 *
 ...
 */
function pickAutoAnswers(questions) {
  const answers = [];
  for (const question of questions) {
    ...
    answers.push({ id: question.id, selected: [stripRecommendedSuffix(chosen.label)] });
  }
  return { answers };
}
...
/**
 * Return the text of the last `user/message` event in a given seq range,
 * or null when none exists. The loop's own followups are user-sourced
 * messages too, so this finds whatever prompt led into a turn — a real
 * user question on round 0, or a queued continuation on later rounds.
 * @param events - the session's durable event log.
 * @param fromSeq - first seq to consider.
 * @param toSeq - first seq to ignore.
 * @returns the last user message text in range, or null.
 */
...
/**
 * Return true when the durable log contains at least one delivered user
 ...
 * @param events - the session's durable event log.
 * @returns true when a user message exists anywhere in the log.
 */
...
var LoopState = class {
  ...
  abort;
  disarmed = false;
  /** True when the driver exited because the user clicked the stop button
   * (last turn ended `aborted`) or the session was archived in the UI.
   * Distinct from a plain teardown: the supervisor keeps halted states in
   * its map so it never reattaches a fresh driver, while toggling the loop
   * off and on sweeps them away to re-arm every conversation (sessions
   * still archived are exempt — a hidden conversation never runs). */
  haltedByUser = false;
...
async function driveLoop(state, logger) {
  ...
  while (!state.disarmed) {
    ...
    if (phaseOutcome.kind === "retry") {
      ...
      const superseded = lastTurnReason(agent.session.events, phaseOutcome.retryAfterSeq) !== undefined;
      lastRoundEndSeq = phaseOutcome.retryAfterSeq;
      if (!superseded) {
        agent.followup(createUserMessage({
          content: [{ type: "text", text: phaseOutcome.retryPrompt }],
          source: { kind: "user" }
        }));
        state.consecutiveFailures += 1;
        logger?.warn?.(`dsh-loop-agent: agent "${agent.id}" retrying round after backoff`);
      } else {
    ...
    if (phaseOutcome.kind === "halt") {
       ...
       * when the user toggles the loop off and on (the enable route sweeps
       * halted states) or dsh restarts. */
      state.haltedByUser = true;
      state.disarm();
      logger?.info?.(`dsh-loop-agent: agent "${agent.id}" endless run stopped by the user`);
      return;
    }
...
function apply(ctx, config) {
  ...
  function installQuestionAutoAnswer() {
    ...
    provider.ask = async function autoAnsweringAsk(request) {
      ...
      if (config.autoAnswerQuestions !== false && drivenAndArmed(agent) && Array.isArray(request?.questions)) {
        const auto = pickAutoAnswers(request.questions);
        if (auto !== null) {
          logger?.info?.(`dsh-loop-agent: agent "${agent.id}" ask auto-answered (${auto.answers.length} question(s), recommended/first option)`);
          return auto;
        }
      }
  ...
  const store = {
    ...
    /** How many conversations currently have a live, armed driver — the
     * sessions the endless loop is attached to and keeps continuing.
     * Halted and parked states (stopped by the user, archived) stay in the
     * map so the supervisor does not reattach them — but they are not
     * "running", so they do not count here. A driver sitting at the
     * disabled gate is not running either: with the global switch off,
     * every live agent parks there, and reporting N "running" agents under
     * an Off switch would lie — count 0 instead. */
    get attachedAgents() {
      if (readDisabled(profile)) return 0;
      let count = 0;
      for (const state of states.values()) if (!state.disarmed) count += 1;
      return count;
    }
  };
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (enabled) {
        /* Re-arm conversations the user previously halted with the stop
         * button: drop their halted states so the 1-second supervisor
         * reattaches a fresh driver (a recent exchange resumes the endless
         * run immediately; a stale one waits for the next user message via
         * the idle-grace guard). Sessions still archived stay halted — an
         * archived conversation must not run, hidden, even after a global
         * toggle. Live drivers are untouched. */
        ...
        for (const [agent, state] of [...states]) {
          ...
          states.delete(agent);
          logger?.info?.(`dsh-loop-agent: loop re-enabled; conversation "${agent.id}" re-armed for the endless run`);
        }
      }
      sendJson(res, 200, {
        ...readSnapshot(),
   ...
   *
   * The supervisor polls once a second, attaches a driver to every agent it
   * has not seen yet, and disarms drivers whose agent has left the registry
   * (the poll replaces the never-delivered `agent/disposed` signal). The
   * same tick consults the workspace archive set: an agent whose session is
   * archived never gets a driver (and a running driver whose session just
   * got archived is halted and its in-flight work cancelled), so archiving
   * a conversation stops its endless run within a second even though the
   * agent stays alive underneath.
    ...
    const timer = setInterval(() => {
      ...
      for (const agent of live) {
        ...
        if (archived?.has(agent.session?.id)) {
           ...
           * happens after the user unarchives AND toggles the loop off/on
           * (the enable sweep skips still-archived sessions) or restarts. */
      ...
      for (const [agent, state] of [...states]) {
        if (!liveSet.has(agent)) {
          state.disarm();
          states.delete(agent);
          logger?.info?.(`dsh-loop-agent: agent "${agent.id}" left registry; loop wound down`);
          continue;
        }
