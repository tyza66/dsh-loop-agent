        ...
        for (const [agent, state] of [...states]) {
          ...
          for (const event of events) {
            ...
            if (isForeverCommand(text)) {
              ...
              state.disarm();
              // Create new armed state and start driver immediately
              const newState = new LoopState(agent, config, profile);
              newState.haltAtSeq = event.seq + 1; // skip past the /forever message
              states.set(agent, newState);
              logger?.info?.(`dsh-loop-agent: agent "${agent.id}" re-armed by /forever command in session events`);
  ...
  ctx.effect(function* () {
    yield ctx.commands.register({
      ...
      handler: async (invocation) => {
        // The actual activation happens in the supervisor sweep.
        // This handler just provides autocomplete and immediate feedback.
        const agent = invocation.agent;
        if (!agent) return { kind: "error", text: "No active agent in this session." };
        if (readDisabled(profile)) return { kind: "error", text: "Endless loop is globally disabled." };

        // Find the state for this agent
        let found = false;
        for (const [mapAgent, mapState] of states) {
          if (mapAgent.id === agent.id) {
            found = true;
            if (!mapState.disarmed) {
              return { kind: "success", text: "Endless loop is already active." };
            }
            break;
          }
        }

        if (!found) return { kind: "error", text: "Session not tracked." };

        // Force supervisor to re-scan by resetting haltAtSeq
        for (const [mapAgent, mapState] of states) {
          if (mapAgent.id === agent.id) {
            mapState.haltAtSeq = Math.max(0, (mapState.haltAtSeq || 0) - 10);
            break;
          }
        }

        return { kind: "success", text: "Endless loop will activate after this message." };
      }
    });
