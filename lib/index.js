...
async function runOnePhase(state, agent, config, logger, cursor) {
  ...
  // `error`). The prompt that triggered the failed round — never the
  // previous successful one — must be re-sent after an exponential backoff;
  ...
  // the success path can later consume. If the very first turn failed
  // before we ever queued a prompt of our own (`lastPromptText` is null),
  // there is nothing to resend; wait for the user to speak instead.
  if (reason?.kind === "error") {
    if (lastPromptText === null) return { kind: "wait" };
